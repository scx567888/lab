package cool.scx.learn.netty.http_proxy.util;
...
public class HttpsSupport {
    ...
    private void initHttpsConfig() {
        try {
            ...
            //从项目目录加入ca私钥
            PrivateKey caPriKey = loadPrivateKey(classLoader.getResource("ca.key"));
     ...
     */
    private PrivateKey loadPrivateKey(URL inputStream) throws Exception {
        Path bytes = Path.of(inputStream.toURI());
        return loadPrivateKey(Files.readAllBytes(bytes), null);
    }

    /**
     * 从文件加载RSA私钥
     */
    private PrivateKey loadPrivateKey(byte[] data, String password) throws Exception {
        JcaPEMKeyConverter converter = new JcaPEMKeyConverter();
        PEMParser pemParser = new PEMParser(new StringReader(new String(data)));
        Object o = pemParser.readObject();
        if (o instanceof PEMEncryptedKeyPair key) {
            PEMKeyPair pemKeyPair = key.decryptKeyPair(new BcPEMDecryptorProvider(password.toCharArray()));
